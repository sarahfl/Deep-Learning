...
def create_pairs(image_path, image_name):
    ...
    for i in tqdm(range(len(image_name))):
        for j in range(i + 1, len(image_name)):
            p = '/home/sarah/Deep-Learning/'
            image_pairs_1.append(p+image_path[i])
            image_pairs_2.append(p+image_path[j])
    ...
    # Save to CSV
    df.to_csv('/home/sarah/Deep-Learning/MS4/data/pairs.csv', index=False)
